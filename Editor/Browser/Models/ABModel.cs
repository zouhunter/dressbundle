...
namespace UFrame.DressBundle.Browser.Models
{
    /// <summary>
    ...
    public static class Model
    {
        ...
        static private Texture2D s_refIcon = null;
        ...
        public static ABDataSource DataSource
        {
            get
            {
                ...
                {
                    s_DataSource = new DressAssetBundleDataSource ();
                }
                return s_DataSource;
            }
        ...
        private static BundleFolderConcreteInfo AddFoldersToBundle(BundleFolderInfo root, BundleNameData nameData)
        {
            ...
            BundleFolderConcreteInfo curretnConcreteInfo = folder;
            var size = nameData.pathTokens.Count;
            for (var index = 0; index < size; index++)
            {
                if (folder != null)
                {
                    var childName = nameData.pathTokens[index];
                    currInfo = folder.GetChild(childName);
                    if (currInfo == null)
                    {
                    ...
                    {
                        curretnConcreteInfo = folder;
                    }
                }
            }
            return curretnConcreteInfo;
        }
        ...
        internal static AssetTreeItem CreateAssetListTreeView(IEnumerable<Browser.Models.BundleInfo> selectedBundles)
        {
        ...
        {
            Debug.LogError("DressBundleBrowser: " + message);
        }
        static internal void LogWarning(string message)
        {
            Debug.LogWarning("DressBundleBrowser: " + message);
        }
        ...
        static internal Texture2D GetBundleIcon(bool refBundle)
        {
            if(refBundle)
            {
                if (s_refIcon == null)
        ...
        static private void FindBundleIcons()
        {
            ...
            s_bundleIcon = (Texture2D)EditorGUIUtility.IconContent("AssetLabelIcon").image;
            s_sceneIcon = (Texture2D)EditorGUIUtility.IconContent("d_Scene").image;
            s_refIcon = (Texture2D)EditorGUIUtility.IconContent("d_UnityEditor.FindDependencies").image;
        }
    }
}
