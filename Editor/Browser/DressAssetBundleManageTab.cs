...
namespace UFrame.DressBundle.Browser
{
    ...
    internal class DressAssetBundleManageTab 
    {
        ...
        internal DressAssetBundleManageTab()
        {
        ...
        {
            UpdateSelectedBundles(new List<Models.BundleInfo>());
            SetSelectedItems(new List<Models.AssetInfo>());
            m_BundleTree.SetSelection(new int[0]);
            Models.Model.ForceReloadData(m_BundleTree);
            m_Parent.Repaint();
        }
        ...
        internal void OnGUI(Rect pos)
        {
            ...
            if (Models.Model.BundleListIsEmpty())
            {
                ...
                GUI.Label(
                    ...
                    new GUIContent(Models.Model.GetEmptyMessage()),
                    style);
            }
            else
            {
        ...
        internal void UpdateSelectedBundles(IEnumerable<Models.BundleInfo> bundles)
        {
            Models.Model.AddBundlesToUpdate(bundles);
            m_AssetList.SetSelectedBundles(bundles);
            m_DetailsList.SetItems(bundles);
            m_MessageList.SetItems(null);
        }

        internal void SetSelectedItems(IEnumerable<Models.AssetInfo> items)
        {
