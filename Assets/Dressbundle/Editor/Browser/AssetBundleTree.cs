...
namespace UFrame.DressBundle.Browser
{
    ...
    { 
        ...
        internal AssetBundleTree(TreeViewState state, DressAssetBundleManageTab ctrl) : base(state)
        {
            Models.Model.Rebuild();
            m_Controller = ctrl;
            showBorder = true;
        }
        ...
        {
            var bundleItem = item as Models.BundleTreeItem;
            return bundleItem.bundle.DoesItemMatchSearch(search);
        }

        protected override void RowGUI(RowGUIArgs args)
        {
            var bundleItem = (args.item as Models.BundleTreeItem);
            ...
            Color old = GUI.color;
            if ((bundleItem.bundle as Models.BundleVariantFolderInfo) != null)
                GUI.color = Models.Model.k_LightGrey; //new Color(0.3f, 0.5f, 0.85f);
            base.RowGUI(args);
            GUI.color = old;
        ...
        { 
            ...
            if (args.newName.Length > 0 && args.newName != args.originalName)
            {
                ...
                Models.BundleTreeItem renamedItem = FindItem(args.itemID, rootItem) as Models.BundleTreeItem;
                args.acceptedRename = Models.Model.HandleBundleRename(renamedItem, args.newName);
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            var root = Models.Model.CreateBundleTreeView();
            return root;
        }
        ...
        protected override void SelectionChanged(IList<int> selectedIds)
        {
            ...
            var selectedBundles = new List<Models.BundleInfo>();
            if (selectedIds != null)
            {
                foreach (var id in selectedIds)
                {
                    var item = FindItem(id, rootItem) as Models.BundleTreeItem;
        ...
        protected override void ContextClicked()
        {
            ...
            List<Models.BundleTreeItem> selectedNodes = new List<Models.BundleTreeItem>();
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            {
                selectedNodes.Add(FindItem(nodeID, rootItem) as Models.BundleTreeItem);
            }
            ...
            {
                if ((selectedNodes[0].bundle as Models.BundleFolderConcreteInfo) != null)
                {
                ...
                else if( (selectedNodes[0].bundle as Models.BundleVariantFolderInfo) != null)
                {
                ...
                {
                    var variant = selectedNodes[0].bundle as Models.BundleVariantDataInfo;
                    if (variant == null)
                    {
                        menu.AddItem(new GUIContent("Add Sibling/New Bundle"), false, CreateNewSiblingBundle, selectedNodes);
                        menu.AddItem(new GUIContent("Add Sibling/New Folder"), false, CreateNewSiblingFolder, selectedNodes);
        ...
        {
            Models.Model.ForceReloadData(this);
        }

        void CreateNewSiblingFolder(object context)
        {
            var selectedNodes = context as List<Models.BundleTreeItem>;
            if (selectedNodes != null && selectedNodes.Count > 0)
            {
                Models.BundleFolderConcreteInfo folder = null;
                folder = selectedNodes[0].bundle.parent as Models.BundleFolderConcreteInfo;
        ...
        void CreateFolder(object context)
        {
            ...
            if (selectedNodes != null && selectedNodes.Count > 0)
            {
                folder = selectedNodes[0].bundle as Models.BundleFolderConcreteInfo;
            }
            CreateFolderUnderParent(folder);
        }
        void CreateFolderUnderParent(Models.BundleFolderConcreteInfo folder)
        {
            var newBundle = Models.Model.CreateEmptyBundleFolder(folder);
        ...
        void CreateBundleUnderParent(Models.BundleFolderInfo folder)
        {
            var newBundle = Models.Model.CreateEmptyBundle(folder);
            ReloadAndSelect(newBundle.nameHashCode, true);
        }
        ...
        void CreateNewSiblingVariant(object context)
        {
            ...
            if (selectedNodes != null && selectedNodes.Count > 0)
            {
                Models.BundleVariantFolderInfo folder = null;
                folder = selectedNodes[0].bundle.parent as Models.BundleVariantFolderInfo;
        ...
        void CreateNewVariant(object context)
        {
            ...
            {
                folder = selectedNodes[0].bundle as Models.BundleVariantFolderInfo;
                CreateVariantUnderParent(folder);
            }
        }
        void CreateVariantUnderParent(Models.BundleVariantFolderInfo folder)
        {
            if (folder != null)
            {
                var newBundle = Models.Model.CreateEmptyVariant(folder);
                ReloadAndSelect(newBundle.nameHashCode, true);
            }
        }
        ...
        void ConvertToVariant(object context)
        {
            ...
            if (selectedNodes.Count == 1)
            {
                var bundle = selectedNodes[0].bundle as Models.BundleDataInfo;
                var newBundle = Models.Model.HandleConvertToVariant(bundle);
                int hash = 0;
                if (newBundle != null)
        ...
        {
            var selectedNodes = context as List<Models.BundleTreeItem>;
            var newBundle = Models.Model.HandleDedupeBundles(selectedNodes.Select(item => item.bundle), onlyOverlappedAssets);
            if(newBundle != null)
            {
        ...
        {
            var selectedNodes = b as List<Models.BundleTreeItem>;
            Models.Model.HandleBundleDelete(selectedNodes.Select(item => item.bundle));
        ...
        class DragAndDropData
        {
            ...
            internal List<Models.BundleInfo> draggedNodes;
            internal Models.BundleTreeItem targetNode;
            ...
            {
                args = a;
                draggedNodes = DragAndDrop.GetGenericData("AssetBundleModel.BundleInfo") as List<Models.BundleInfo>;
                targetNode = args.parentItem as Models.BundleTreeItem;
                ...
                if (draggedNodes != null)
                {
                    ...
                    {
                        if ((bundle as Models.BundleFolderInfo) != null)
                        {
                            hasBundleFolder = true;
                        }
                        else
                        {
                            var dataBundle = bundle as Models.BundleDataInfo;
                            if (dataBundle != null)
                            {
                                ...
                                if ( (dataBundle as Models.BundleVariantDataInfo) != null)
            ...
            switch (args.dragAndDropPosition)
            {
                ...
                case DragAndDropPosition.OutsideItems:
                    if (data.draggedNodes != null)
                    {
                        ...
                        {
                            Models.Model.HandleBundleReparent(data.draggedNodes, null);
                            Reload();
                        }
                    }
        ...
        private DragAndDropVisualMode HandleDragDropUpon(DragAndDropData data)
        {
            ...
            var targetDataBundle = data.targetNode.bundle as Models.BundleDataInfo;
            if (targetDataBundle != null)
            {
                ...
                if (data.args.performDrop)
                {
                    if (data.draggedNodes != null)
                    {
                        Models.Model.HandleBundleMerge(data.draggedNodes, targetDataBundle);
                        ReloadAndSelect(targetDataBundle.nameHashCode, false);
                    }
                    else if (data.paths != null)
                    {
                        Models.Model.MoveAssetToBundle(data.paths, targetDataBundle.m_Name.bundleName, targetDataBundle.m_Name.variant);
            ...
            {
                var folder = data.targetNode.bundle as Models.BundleFolderInfo;
                if (folder != null)
                {
                    if (data.args.performDrop)
                    {
                        if (data.draggedNodes != null)
                        {
                            Models.Model.HandleBundleReparent(data.draggedNodes, folder);
                            Reload();
                        }
                        else if (data.paths != null)
                        {
        ...
        private DragAndDropVisualMode HandleDragDropBetween(DragAndDropData data)
        {
            ...
            var parent = (data.args.parentItem as Models.BundleTreeItem);

            if (parent != null)
            {
                var variantFolder = parent.bundle as Models.BundleVariantFolderInfo;
                if (variantFolder != null)
                    return DragAndDropVisualMode.Rejected;

                if (data.args.performDrop)
                {
                    var folder = parent.bundle as Models.BundleFolderConcreteInfo;
                    if (folder != null)
                    {
                        if (data.draggedNodes != null)
                        {
        ...
        private Models.BundleFolderInfo dragToNewSpaceRoot = null;
        private void DragPathsAsOneBundle()
        {
            var newBundle = Models.Model.CreateEmptyBundle(dragToNewSpaceRoot);
            Models.Model.MoveAssetToBundle(dragToNewSpacePaths, newBundle.m_Name.bundleName, newBundle.m_Name.variant);
            Models.Model.ExecuteAssetMove();
            ReloadAndSelect(newBundle.nameHashCode, true);
        }
        private void DragPathsAsManyBundles()
        {
            ...
            {
                var newBundle = Models.Model.CreateEmptyBundle(dragToNewSpaceRoot, System.IO.Path.GetFileNameWithoutExtension(assetPath)/*.ToLower()*/);
                Models.Model.MoveAssetToBundle(assetPath, newBundle.m_Name.bundleName, newBundle.m_Name.variant);
        ...
        private void DragPathsToNewSpace(string[] paths, Models.BundleFolderInfo root)
        {
