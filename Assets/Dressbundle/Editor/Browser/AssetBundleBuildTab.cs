...
using UFrame.DressBundle.Browser.AssetBundleDataSource;
...
namespace UFrame.DressBundle.Browser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void OnDisable()
        {
            ...
            dataPath += "/Library/DressAssetBundleBrowserBuild.asset";
            EditorUtility.SetDirty(m_UserData);
            UnityEditorInternal.InternalEditorUtility.SaveToSerializedFileAndForget(new Object[] { m_UserData }, dataPath,true);
        }
        internal void OnEnable(EditorWindow parent)
        {
            m_InspectTab = (parent as DressAssetBundleBrowserMain).m_InspectTab;
            ...
            {
                var objs = UnityEditorInternal.InternalEditorUtility.LoadSerializedFileAndForget(dataPath);
                if (objs != null && objs.Length > 0)
                {
                    m_UserData = objs[0] as BuildTabData;
                }
            }
            if(m_UserData ==null)
                m_UserData = ScriptableObject.CreateInstance<BuildTabData>();
        ...
        internal void OnGUI()
        {
            ...
            // build target
            using (new EditorGUI.DisabledScope (!Models.Model.DataSource.CanSpecifyBuildTarget)) {
            ...
            ////output path
            using (new EditorGUI.DisabledScope (!Models.Model.DataSource.CanSpecifyBuildOutputDirectory)) {
                EditorGUILayout.Space();
            ...
            // advanced options
            using (new EditorGUI.DisabledScope (!Models.Model.DataSource.CanSpecifyBuildOptions)) {
                EditorGUILayout.Space();
        ...
        {
            if (Models.Model.DataSource.CanSpecifyBuildOutputDirectory) {
            ...
            if (Models.Model.DataSource.CanSpecifyBuildOptions) {
            ...
            Models.Model.DataSource.BuildAssetBundles (buildInfo);
        ...
        [System.Serializable]
        public class BuildTabData:ScriptableObject
        {
            internal List<string> m_OnToggles = new List<string>();
